...
for d = 1:length(dampingFolders)
    ...
    for k = 1:length(files)
        ...
        ch1_raw = data(:,2);   % CH1 = LVDT connected to base excitation
        ch2_raw = data(:,3);   % CH2 = Magnetostrictive response

        f_nominal = fileFreqs(k);
        ...
        f = estimateDriveFreqFFT(t, ch1, f_nominal);
        r = f / fn;
        ...
        % Store results
        results = [results; f_nominal, f, r, phase_rawSignal, phase_corrected];

        allResults = [allResults; ...
            d, f_nominal, f, r, phase_rawSignal, phase_corrected];
        ...
        title(sprintf('MEAS: f = %.3f Hz, r = %.2f, phase = %.1f°', ...
       ...
       'CH2 measured response', ...
        ...
        title(sprintf('PROC: f = %.3f Hz, r = %.2f, phase = %.1f°', ...
    ...
    resultsTable = array2table(results, ...
        'VariableNames', ...
        {'Nominal_Frequency_Hz', 'FFT_Frequency_Hz', 'Frequency_Ratio', ...
    ...
    plot(results(:,3), results(:,4), 'o--', 'LineWidth', 1.2); hold on;
    plot(results(:,3), results(:,5), 'o-',  'LineWidth', 1.5);
...
allResults = sortrows(allResults, [1 3]);

allResultsTable = array2table(allResults, ...
    'VariableNames', ...
    {'DampingFolderIndex', 'Nominal_Frequency_Hz', 'FFT_Frequency_Hz', ...
     'Frequency_Ratio', 'Raw_Phase_deg', 'Corrected_Phase_deg'});
...
for d = 1:length(dampingFolders)
    ...
    plot(temp(:,4), temp(:,6), 'o-', 'LineWidth', 1.5, ...
...
for d = 1:length(dampingFolders)
    ...
    plot(temp(:,4), temp(:,5), 'o--', 'LineWidth', 1.5, ...
        'DisplayName', legendLabels{d});
...
function f_est = estimateDriveFreqFFT(t, refSignal, f_nominal)

    dt = mean(diff(t));
    Fs = 1 / dt;
    N  = length(t);

    refSignal = refSignal - mean(refSignal, 'omitnan');
    ...
    searchIdx = freqAxis >= (f_nominal - searchWidth) & ...
                freqAxis <= (f_nominal + searchWidth) & ...
                freqAxis > 0 & ...
                freqAxis <= Fs/2;

    if ~any(searchIdx)
        warning('No FFT bins found near %.2f Hz. Using nominal frequency.', f_nominal);
        f_est = f_nominal;
        return;
    end
    ...
    searchFreqs = freqAxis(searchIdx);
    searchMag   = mag(searchIdx);

    [~, localIdx] = max(searchMag);

    f_est = searchFreqs(localIdx);
